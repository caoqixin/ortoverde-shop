            ...
            <div class="card">
                ...
                <!-- 输出后端报错结束 -->
                    ...
                    <user-address-create-and-edit inline-template>
                        @if($address->id)
                            <form action="{{ route('user_addresses.update', ['user_address' => $address->id]) }}"
                                ...
                                @else
                                    ...
                                    <!-- 注意这里多了 @change -->
                                        <select-town
                                                :init-value="{{ json_encode([old('region', $address->region), old('province', $address->province), old('town', $address->town)]) }}"
                                                @change="onTownChanged" inline-template>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-sm-3">
                                                    <select class="form-control" v-model="regionId">
                                                        ...
                                                        <option v-for="(name, id) in regions" :value="id">@{{ name }}
                                                        </option>
                                                    </select>
                                                </div>
                                                <div class="col-sm-3">
                                                ...
                                                <div class="col-sm-3">
                                                    <select class="form-control" v-model="townId">
                                                        <option value="">选择城市</option>
                                                        <option v-for="(name, id) in towns" :value="id">@{{ name }}
                                                        </option>
                                                    </select>
                                                </div>
                                            </div>
