...
class Migrate extends Command
{
    ...
    {
        // 暂时关闭索引
        $this->es->indeces()->close(['index' => $aliasName]);

        // 更新索引设置
        $this->es->indeces()->putSettings([
            'index' => $aliasName,
        ...
        $this->es->indices()->putMapping([
            'index' => $aliasName,
            'body' => [
                'properties' => $indexClass::getPropeties(),
            ],
        ]);

        // 重新打开索引
        $this->es->indeces()->open(['index' => $aliasName]);
    }
    ...
    protected function reCreateIndex($aliasName, $indexClass)
    {
        ...
        try {
            ...
            if ($this->es->indices()->exists(['index' => $newIndexName])) {
                $this->warn($newIndexName . "已存在");
                $this->info("正在删除该索引");
                $this->es->indices()->delete(['index' => $newIndexName]);
                $this->info("删除成功");

                $this->info("正在重新创建索引" . $aliasName);
                $this->es->indices()->create([
                    'index' => $newIndexName,
                    'body' => [
                        'settings' => $indexClass::getSettings(),
